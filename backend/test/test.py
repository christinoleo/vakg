...
from app.api.knowledge.knowledge import State, Sequence, Sequence
...
def req(*, h: Optional[Sequence], m: Optional[Sequence], graph: int = 0):
    ...
    if h is not None:
        data["h_sequence"] = h.dict()
    if m is not None:
        data["c_sequence"] = m.dict()
    print(data)
...
def get_suggestions(text: str, filter: str = "intention"):
    r = requests.post(
        "http://localhost:8080/nlp/similar/" + text,
        json=dict(filters=dict(label=filter)),
        timeout=0.5,
    ).json()
    return r
...
def new_machine(state: StateUpdate, update: StateUpdate, graph=0, user=1, analysis=1):
    req(
        graph=graph,
        m=Sequence(
            state=State(label=state.label, data=flatten_dict(state.data)),
            sequence=Sequence(
                label=update.label,
                data=flatten_dict(update.data),
                user=user,
                analysis=analysis,
            ),
        ),
...
def new_user(state: StateUpdate, update: StateUpdate, graph=0, user=1, analysis=1):
    req(
        graph=graph,
        h=Sequence(
            state=State(label=state.label, data=state.data),
            sequence=Sequence(
                label=update.label, data=update.data, user=user, analysis=analysis
            ),
        ),
        m=None,
    )
...
r = requests.post(
    "http://localhost:8080/nlp/similar/" + text,
    json=dict(filter=dict(label="insight")),
    timeout=0.5,
).json()
